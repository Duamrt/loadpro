/**
 * Muscle Map - LoadPro
 * SVG polygon data extracted from react-body-highlighter (giavinh79)
...
// ─── Mapeamento LoadPro (PT-BR) → IDs SVG ───
const MUSCLE_MAP = {
  'Peito': ['chest'],
  'Costas': ['upper-back'],
  ...
  'Trapezio': ['trapezius'],
  'Lombar': ['lower-back'],
  'Adutor': ['adductor', 'abductors'],
};

// ─── Render ───
function renderMuscleMap(containerId, primaryMuscle, secondaryMuscles = []) {
  ...
  if (primaryMuscle && MUSCLE_MAP[primaryMuscle]) {
    MUSCLE_MAP[primaryMuscle].forEach(id => primaryIds.add(id));
  }
  secondaryMuscles.forEach(name => {
    if (MUSCLE_MAP[name]) {
      MUSCLE_MAP[name].forEach(id => {
